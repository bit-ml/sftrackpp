...
class TrackingNetDataset(BaseDataset):
    ...
    def _list_sequences(self, root, set_ids):
        ...
        for s in set_ids:
            ...
            if self.split == "train_few":
                # rsamp = random.sample(sequences_cur_set, 50)
                with open('{}/trackingnet_train_few_split.txt'.format(
                        root)) as f:
                    seq_ids = f.read().splitlines()
                sequences_cur_set = [xy_str.split(", ") for xy_str in seq_ids]
            else:
